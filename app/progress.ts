...
export function parseProgress(raw: string | null, ids: string[]): Progress {
  ...
  for (const id of ids) {
    ...
    if (!e) continue;
    if (typeof e.started !== "boolean" || typeof e.saved !== "boolean" || typeof e.completed !== "boolean" || !Array.isArray(e.answers) || !e.answers.every((a: unknown)=>a===0||a===1) || !Number.isInteger(e.stage) || e.stage<0 || e.stage>3 || (e.due!==null && (typeof e.due!=="string" || !/^\d{4}-\d{2}-\d{2}$/.test(e.due) || Number.isNaN(Date.parse(e.due))))) throw new Error("Invalid lesson progress");
