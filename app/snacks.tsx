...
export default function Snacks() {
  const [theme,setTheme] = useState<"normal"|"gaming">("normal");
  const [themeWarning,setThemeWarning] = useState("");
  useEffect(()=>{
    try {
      const saved=localStorage.getItem("system-snacks:theme");
      if(saved==="gaming") { setTheme(saved); document.documentElement.dataset.theme=saved; }
    } catch { setThemeWarning("Theme preferences can’t be saved in this browser. You can still switch themes."); }
  },[]);
  function selectTheme(next:"normal"|"gaming") {
    setTheme(next);
    document.documentElement.dataset.theme=next;
    try { localStorage.setItem("system-snacks:theme",next); setThemeWarning(""); }
    catch { setThemeWarning("Your theme changed, but this browser can’t save it for your next visit."); }
  }
